...
for FILE in $CHANGED_FILES; do
  ...
  if [[ $FILE == *Test.cls ]]; then
    find $classPath -samefile "$FILE-meta.xml" -exec sudo cp -t --parents $DEPLOYDIR {} +
    ...
    find $classPath -samefile "$FILE2.cls" -exec sudo cp -t --parents $DEPLOYDIR {} +
    ...
    find $triggerPath -samefile "$FILE-meta.xml" -exec sudo cp -t --parents $DEPLOYDIR {} +
    ...
    find $triggerPath -samefile "$FILE3.trigger" -exec sudo cp -t --parents $DEPLOYDIR {} +
