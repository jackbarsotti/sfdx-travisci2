...
const _isAllowlistedForDecomposition = function (typeDef) {
...
const _augmentMetadataInfosWithDecompositions = function (typeDefs) {
    return _getMetadataWsdlJson().then(json => {
        Object.keys(typeDefs).forEach(metadataName => {
            ...
            if (_isAllowlistedForDecomposition(typeDef) && _hasChildXmlNames(typeDef)) {
...
const _getMetadataWsdlJson = function () {
    ...
    return core_1.fs.readFile(metadataWsdlPath, 'utf8').then(wsdl => xml2jsParseString(wsdl, { explicitArray: false }));
};
...
class MetadataRegistry {
    ...
    getTypeDefinitionByFileNameWithCoresidentMetadataFile(fileName, typeDefsToCheck, recurse) {
        ...
        const typeDef = typeDefsToCheck.find(typeDef => {
            return srcDevUtil.pathExistsSync(path.join(dir, `${fullName}.${typeDef.ext}${this.metadataFileExt}`));
        });
        if (!util.isNullOrUndefined(typeDef)) {
            return typeDef;
        }
