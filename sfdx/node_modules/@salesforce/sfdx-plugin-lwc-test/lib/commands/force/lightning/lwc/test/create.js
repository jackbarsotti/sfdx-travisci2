...
class Create extends command_1.SfdxCommand {
    async run() {
        ...
        if (path.extname(modulePath) !== '.js') {
            throw new core_1.SfdxError(messages.getMessage('errorFileNotJs', [this.flags.filepath]));
        }
        if (!fs.existsSync(modulePath)) {
            throw new core_1.SfdxError(messages.getMessage('errorFileNotFound', [this.flags.filepath]));
        }
        const htmlPath = modulePath.substring(0, modulePath.lastIndexOf('.')) + '.html';
        if (!fs.existsSync(htmlPath)) {
            throw new core_1.SfdxError(messages.getMessage('errorHtmlFileNotFound', [this.flags.filepath]));
        }
        ...
        if (fs.existsSync(testPath)) {
            throw new core_1.SfdxError(messages.getMessage('errorFileExists', [testPath]));
        }
